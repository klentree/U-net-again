...
def load_dataset():
    ...
    train = config.get("data_train")
    val = config.get("data_val")
    test = config.get("data_test")
...
av_time_img = av_time_epoch/num_img #количество изображений в эпохе (надо переделать проценты)
